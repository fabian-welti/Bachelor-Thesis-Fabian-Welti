...
#Calendar Spread 2 
# with contango backwardation trigger
...
calspread2 <- subset(vix_futures_06_24, Date >= as.Date("2006-01-03") & Date <= as.Date("2024-01-12"))
calspread2 <- subset(calspread2, select = -c(High, Low, `Total Volume`, EFP, Change, `Open Interest`, `Futures expiration month`, `Expiration Year`, `Expiration month and year`))
...
calspread2 <- merge(treasury_data, calspread2, by="Date", all.x = TRUE)
calspread2$RFR <- as.numeric(calspread2$RFR)
calspread2 <- calspread2[order(calspread2$Date, calspread2$ExpirationDate), ] #order the data

# read in the data for the vix index for the same time period
vix_data_calspread2 <- read_excel("VIX_History 1990-2024.xlsx", skip = 1) # Adjust path as necessary
vix_data_calspread2$Date <- as.Date(vix_data_calspread2$Date, format = "%m/%d/%Y")
vix_data_calspread2 <- subset(vix_data_calspread2, Date >= as.Date("2006-01-03"))


# combine the datasets for the final dataset
calspread2 <- merge(calspread2, vix_data_calspread2, by = "Date", suffixes = c("_futures", "_index"))
# Convert the RFR from percentage to a daily rate by dividing by 360
calspread2$RFR <- calspread2$RFR / 36000 # Dividing by 100 to get rate and by 360 for daily rate
...
# Clean the data since some data for second and third month contracts is not complete
# Dates to remove
dates_to_remove <- as.Date(c("2006-01-18", "2006-01-19", "2006-02-15", "2006-02-16", "2006-04-19", "2006-04-20", 
                             "2006-05-17", "2006-05-18", "2006-07-19", "2006-07-20", "2006-07-21", "2006-03-23",
                             "2006-08-16", "2006-08-17", "2007-11-21", "2006-01-20", "2006-01-23", "2006-02-17",
                             "2006-02-21", "2006-04-21", "2006-05-19", "2006-08-18", "2008-02-19", "2014-03-18" ))

# Filtering out the specified dates
calspread2 <- calspread2 %>%
  filter(!as.Date(Date) %in% dates_to_remove)

# Clean the date on the 23.02.2006
calspread2$Close_futures[56] <- calspread2$Settle[56]
calspread2$Open_futures[56] <- calspread2$Settle[56]
...
calspread2$RebalanceDay[1] <- TRUE
calspread2$RebalanceDay[2] <- TRUE
...
#Step 4: new column calculating the PnL of the roll for each leg of the spread introducing the contango backwardation trigger####
# Step 4.1: create new column for the term structure: 
calspread2 <- calspread2 %>%
  mutate(
    # Calculate TermStructure only for odd-numbered rows
    TermStructure = ifelse(
      row_number() %% 2 != 0,  # Check if the row number is odd
      ifelse(
        lag(Close_futures, n = 2) > lag(Close_index, n = 2), 
        "Contango", 
        "Backwardation"
      ), 
      NA  # NA for even rows initially
    )
  ) %>%
  # Fill NA values in even rows with the value from the odd row just above
  fill(TermStructure, .direction = "down")

# Manually set the TermStructure for the first row to "Contango"
calspread2$TermStructure[1] <- "Contango"
calspread2$TermStructure[2] <- "Contango"



# Step 4.2. calculate the daily PnL for each leg of the spread seperatly
# Loop through the dataset, now considering every two rows as a pair for each day
for(i in 1:nrow(calspread2)) {
  if(i == 1 || calspread2$RebalanceDay[i]) {
    # Determine positions based on TermStructure
    position <- if_else(calspread2$TermStructure[i] == "Contango", -1, 1)
    calspread2$CurrentPosition[i] <- position
    # Ensure the second contract of the day takes the opposite position
    if(i < nrow(calspread2)) calspread2$CurrentPosition[i + 1] <- -position
  } else {
    # Carry over the previous position if TermStructure hasn't changed
    if(i > 2 && calspread2$TermStructure[i] != calspread2$TermStructure[i - 2]) {
      calspread2$CurrentPosition[i] <- -calspread2$CurrentPosition[i - 2]
    } else {
      calspread2$CurrentPosition[i] <- calspread2$CurrentPosition[i - 2]
    }
  }
  ...
  # Calculate PnL
  if(i < nrow(calspread2) - 1) {
    calspread2$DailyPnL[i] <- calspread2$CurrentPosition[i] * (calspread2$Open_futures[i + 2] - calspread2$Open_futures[i])
  }
  
  # Handle the last two rows and expiration days
  if(i >= nrow(calspread2) - 1 || calspread2$IsExpirationNextDay[i]) {
    calspread2$DailyPnL[i] <- calspread2$CurrentPosition[i] * (calspread2$Settle[i] - calspread2$Open_futures[i])
  }
}

# Adjust the sign from the PnL in the second leg (second expiration month) 
for(i in 2:nrow(calspread2)) {
  if(i %% 2 == 0) {  # Check if the row index is even
    calspread2$DailyPnL[i] <- -calspread2$DailyPnL[i]  # Change the sign of DailyPnL
  }
}
...
Results_calspread2 <- calspread2 %>%
  ...
  summarise(
    ...
    TermStructure = first(TermStructure)
  ) %>%
  ungroup()
...
# Prepare the dataset by predicting if the next day is a Rebalance Day
Results_calspread2 <- Results_calspread2 %>%
  mutate(
...
for (i in 1:nrow(Results_calspread2)) {
  if (i == 1) {
    Results_calspread2$MarginAccount[i] <- Mt
    Results_calspread2$PositionSize[i] <- ps
  } else {
    # Calculate Margin Account before applying costs
    Mt_next <- (1 + Results_calspread2$RFR[i-1]) * Results_calspread2$MarginAccount[i-1] +
      Results_calspread2$PositionSize[i-1] * Results_calspread2$roll_pnl[i]
    
    # Update Position Size on Rebalance Days before applying costs
    if (Results_calspread2$RebalanceDay[i]) {
      ps <- floor(Mt_next / 50000)
    }
    
    # Apply trading costs on Rebalance Days
    if (Results_calspread2$RebalanceDay[i]) {
      Mt_next <- Mt_next - 2.32 * ps  # Subtract fixed trading costs
    }
    
    # Apply spread costs if RebalanceDay is today or tomorrow
    if (Results_calspread2$RebalanceDay[i] || Results_calspread2$NextRebalance[i]) {
      Mt_next <- Mt_next - 0.05 * 1000 * ps  # Subtract bid-ask spread costs
    }
    
    # Assign the updated values to the dataset after position size and cost adjustments
    Results_calspread2$MarginAccount[i] <- Mt_next
    Results_calspread2$PositionSize[i] <- ps
    Mt <- Mt_next
  }
}
...
# Import the data
margin_requirement_5 <- read_excel("margin requirements vix futures contback1.xlsx")

# Prepare the margin requirement data by converting Date, selecting needed columns, and arranging by Date
margin_requirement_5 <- margin_requirement_5 %>%
  mutate(Date = as.Date(Date, format = "%Y-%m-%d")) %>%  # Ensure the date format matches your dataset
  select(Date, IM, MM) %>%
  arrange(Date)

# Merge the margin requirement data with Results_calspread2
Results_calspread2 <- Results_calspread2 %>%
  left_join(margin_requirement_5, by = "Date") %>%
...
Results_calspread2 <- Results_calspread2 %>%
  mutate(
    `IB IM` = case_when(
      TermStructure == "Contango" ~ (IM * 1.8)*0.4,
      TermStructure == "Backwardation" ~ (IM * 1.8)*0.4,
      TRUE ~ IM  # Default case if there are other values or NA
    ),
    `IB MM` = case_when(
      TermStructure == "Contango" ~ (MM * 1.8)*0.4,
      TermStructure == "Backwardation" ~ (MM * 1.8)*0.4,
...
Results_calspread2 <- Results_calspread2 %>%
  mutate(margin_requirement_5 = ifelse(RebalanceDay,
                                       PositionSize * `IB IM`,
                                       PositionSize * `IB MM`))
...
Results_calspread2$daily_pnl_lastprice[1] <- 0
# Replace NA values in 'daily_pnl_lastprice' with 0
Results_calspread2$daily_pnl_lastprice <- replace_na(Results_calspread2$daily_pnl_lastprice, 0)

# Calculate the initial contract size based on "Last Price"
x_lastprice <- round(1000000 / Results_calspread2$`Last Price`[1], digits = 0)

# Initialize the 'VPD' column with the first value as the initial investment
Results_calspread2$VPD <- rep(0, nrow(Results_calspread2))
Results_calspread2$VPD[1] <- 1000000  # Set the initial value for the first row

# Loop through the dataset starting from the second row to calculate VPD
for (i in 2:nrow(Results_calspread2)) {
  Results_calspread2$VPD[i] <- x_lastprice * Results_calspread2$daily_pnl_lastprice[i] + Results_calspread2$VPD[i - 1]
}
...
Results_calspread2$daily_pnl_spx[1] <- 0
# Replace NA values in 'daily_pnl_spx' with 0
Results_calspread2$daily_pnl_spx <- replace_na(Results_calspread2$daily_pnl_spx, 0)

# Calculate the development of 100k 
# Calculate the initial contract size 'x' using the correct column name "PUT Index"
x <- round(1000000 / Results_calspread2$Close_SPX[1], digits = 0)

# Initialize the 'SPX' column with the first value as the initial investment
Results_calspread2$SPX <- rep(0, nrow(Results_calspread2))
Results_calspread2$SPX[1] <- 1000000  # Set the initial value for the first row

# Loop through the dataset starting from the second row
for (i in 2:nrow(Results_calspread2)) {
  Results_calspread2$SPX[i] <- x * Results_calspread2$daily_pnl_spx[i] + Results_calspread2$SPX[i - 1]
}
...
upper_limit <- max(c(max(Results_calspread2$MarginAccount, na.rm = TRUE), 
                     max(Results_calspread2$margin_requirement_5, na.rm = TRUE),
                     max(Results_calspread2$VPD, na.rm = TRUE),
                     max(Results_calspread2$SPX, na.rm = TRUE)))
...
plot(Results_calspread2$Date, Results_calspread2$MarginAccount, type = 'l', 
     col = "darkblue", ylim = y_range, xaxt = 'n',    # Disable default x-axis
     xlab = "", ylab = "Development of $1'000'000", 
     main = "Performance CalSpread2", lwd = 2)  # Increase line width with lwd

# Add the remaining series with lines
lines(Results_calspread2$Date, Results_calspread2$margin_requirement_5, col = "deepskyblue4", lwd = 2)
lines(Results_calspread2$Date, Results_calspread2$VPD, col = "red", lwd = 2)
lines(Results_calspread2$Date, Results_calspread2$SPX, col = "black", lwd = 2)

# Custom x-axis labels for each year
year_ticks <- seq(from=min(Results_calspread2$Date, na.rm=TRUE),
                  to=max(Results_calspread2$Date, na.rm=TRUE), by="years")
...
legend("topleft", legend = c("CalSpread2", "Margin Requirement CalSpread2", "VPD Index", "S&P 500"),
...
#Descriptive Statistics CalSpread2 ####
...
Results_calspread2$Returns[1] <- 0  


# Step 2: Generate Summary Statistics
summary_stats_calspread2 <- Results_calspread2 %>%
  summarise(N.Obs = n(),
...
peak = Results_calspread2$MarginAccount[1]
max_drawdown = 0

# Loop through the MarginAccount to find the maximum drawdown
for (value in Results_calspread2$MarginAccount) {
...
skewness <- skewness(Results_calspread2$Returns, na.rm = TRUE)
kurtosis <- kurtosis(Results_calspread2$Returns, na.rm = TRUE)


# Step 5: Calculate Cumulative Return
cumulative_return <- last(Results_calspread2$MarginAccount) / first(Results_calspread2$MarginAccount) - 1


# Step 6: Annualize Returns and Volatility
annualized_return <- (1 + mean(Results_calspread2$Returns, na.rm = TRUE)) ^ 252 - 1  # Assuming 252 trading days in a year
annualized_volatility <- sd(Results_calspread2$Returns, na.rm = TRUE) * sqrt(252)



# Step 7: Calculate the Sharpe Ratio
# Calculate the Daily Excess Returns
Results_calspread2$ExcessReturns <- Results_calspread2$Returns - Results_calspread2$RFR

# Calculate the Mean Daily Excess Retun and Standard Deviation for the Daily Returns 
mean_daily_excess_return_calspread2 <- mean(Results_calspread2$ExcessReturns, na.rm = TRUE)
sd_daily_excess_returns_calspread2 <- sd(Results_calspread2$Returns, na.rm = TRUE)

# Compute the Daily Sharpe Ratio
daily_sharpe_ratio_calspread2 <- mean_daily_excess_return_calspread2 / sd_daily_excess_returns_calspread2

# Annualize the Sharpe Ratio
annualized_sharpe_ratio_calspread2 <- daily_sharpe_ratio_calspread2 * sqrt(252)

annualized_sharpe_ratio_calspread2


# Step 8: Calculate the Sortino Ratio
# Calculate the Downside Deviation
squared_negative_excess_returns <- ifelse(Results_calspread2$ExcessReturns < 0, 
                                          Results_calspread2$ExcessReturns^2, 
                                          0)
...
daily_sortino_ratio <- mean_daily_excess_return_calspread2 / downside_deviation
...
annualized_mean_daily_excess_return <- mean(Results_calspread2$ExcessReturns, na.rm = TRUE) * 252

# Compute the Calmar Ratio using the Annualized Mean of Daily Excess Returns
calmar_ratio_calspread2 <- annualized_mean_daily_excess_return / max_drawdown

# The result is the Calmar Ratio for your strategy
calmar_ratio_calspread2



# Combine all the results into a single tibble
descrip_stats_results_calspread2 <- tibble(
  ...
  CalSpread2 = c(summary_stats_calspread2$Mean, summary_stats_calspread2$Median, summary_stats_calspread2$S.D., summary_stats_calspread2$Min, summary_stats_calspread2$Max, 
                 max_drawdown, skewness, kurtosis, cumulative_return, annualized_return, annualized_volatility, annualized_sharpe_ratio_calspread2, annualized_sortino_ratio, calmar_ratio_calspread2)
)

# make the dataset visually nicer
# Convert tibble to data frame if it is not already
descrip_stats_results_calspread2 <- as.data.frame(descrip_stats_results_calspread2)

# Set the row names and remove the 'Statistics' column
rownames(descrip_stats_results_calspread2) <- descrip_stats_results_calspread2$Statistics
descrip_stats_results_calspread2 <-descrip_stats_results_calspread2[-1]

# Round the numbers in the CalSpread2 column to 2 decimal places
descrip_stats_results_calspread2$CalSpread2 <- round(descrip_stats_results_calspread2$CalSpread2, 4)

# Print the results to check
print(descrip_stats_results_calspread2)
...
Results_calspread2$Returns_VPD[1] <- 0  


# Step 2: Generate Summary Statistics
summary_stats_vpd <- Results_calspread2 %>%
  summarise(N.Obs = n(),
...
# Step 3: Calculate Max Drawdown
# Initialize variables to track the peak and maximum drawdown
peak = Results_calspread2$VPD[1]
max_drawdown = 0

# Loop through the VPD to find the maximum drawdown
for (value in Results_calspread2$VPD) {
  # Update the peak if the current value is higher than the current peak
  if (value > peak) {
    peak = value
  }
...
# Step 4: Compute Skewness and Kurtosis
skewness <- skewness(Results_calspread2$Returns_VPD, na.rm = TRUE)
kurtosis <- kurtosis(Results_calspread2$Returns_VPD, na.rm = TRUE)


# Step 5: Calculate Cumulative Return
cumulative_return <- last(Results_calspread2$VPD) / first(Results_calspread2$VPD) - 1


# Step 6: Annualize Returns_VPD and Volatility
annualized_return <- (1 + mean(Results_calspread2$Returns_VPD, na.rm = TRUE)) ^ 252 - 1  # Assuming 252 trading days in a year
annualized_volatility <- sd(Results_calspread2$Returns_VPD, na.rm = TRUE) * sqrt(252)


# Step 7: Calculate the Sharpe Ratio
# Calculate the Daily Excess Returns_VPD
Results_calspread2$ExcessReturns_VPD <- Results_calspread2$Returns_VPD - Results_calspread2$RFR

# Calculate the Mean Daily Excess Retun and Standard Deviation for the Daily Returns_VPD 
mean_daily_excess_return_vpd <- mean(Results_calspread2$ExcessReturns_VPD, na.rm = TRUE)
sd_daily_excess_Returns_VPD <- sd(Results_calspread2$Returns_VPD, na.rm = TRUE)
...
# Step 8: Calculate the Sortino Ratio
# Calculate the Downside Deviation
squared_negative_excess_Returns_VPD <- ifelse(Results_calspread2$ExcessReturns_VPD < 0, 
                                              Results_calspread2$ExcessReturns_VPD^2, 
                                              0)
...
annualized_mean_daily_excess_return <- mean(Results_calspread2$ExcessReturns_VPD, na.rm = TRUE) * 252
...
Results_calspread2$Returns_SPX[1] <- 0  


# Step 2: Generate Summary Statistics
summary_stats_spx <- Results_calspread2 %>%
  summarise(N.Obs = n(),
...
# Step 3: Calculate Max Drawdown
# Initialize variables to track the peak and maximum drawdown
peak = Results_calspread2$SPX[1]
max_drawdown = 0

# Loop through the SPX to find the maximum drawdown
for (value in Results_calspread2$SPX) {
  # Update the peak if the current value is higher than the current peak
  if (value > peak) {
    peak = value
  }
...
# Step 4: Compute Skewness and Kurtosis
skewness <- skewness(Results_calspread2$Returns_SPX, na.rm = TRUE)
kurtosis <- kurtosis(Results_calspread2$Returns_SPX, na.rm = TRUE)


# Step 5: Calculate Cumulative Return
cumulative_return <- last(Results_calspread2$SPX) / first(Results_calspread2$SPX) - 1


# Step 6: Annualize Returns_SPX and Volatility
annualized_return <- (1 + mean(Results_calspread2$Returns_SPX, na.rm = TRUE)) ^ 252 - 1  # Assuming 252 trading days in a year
annualized_volatility <- sd(Results_calspread2$Returns_SPX, na.rm = TRUE) * sqrt(252)


# Step 7: Calculate the Sharpe Ratio
# Calculate the Daily Excess Returns_SPX
Results_calspread2$ExcessReturns_SPX <- Results_calspread2$Returns_SPX - Results_calspread2$RFR

# Calculate the Mean Daily Excess Retun and Standard Deviation for the Daily Returns_SPX 
mean_daily_excess_return_spx <- mean(Results_calspread2$ExcessReturns_SPX, na.rm = TRUE)
sd_daily_excess_Returns_SPX <- sd(Results_calspread2$Returns_SPX, na.rm = TRUE)
...
# Step 8: Calculate the Sortino Ratio
# Calculate the Downside Deviation
squared_negative_excess_Returns_SPX <- ifelse(Results_calspread2$ExcessReturns_SPX < 0, 
                                              Results_calspread2$ExcessReturns_SPX^2, 
                                              0)
...
annualized_mean_daily_excess_return <- mean(Results_calspread2$ExcessReturns_SPX, na.rm = TRUE) * 252
...
model_calspread2 <- lm(Returns ~ Returns_SPX, data = Results_calspread2)
beta_calspread2 <- coef(model_calspread2)["Returns_SPX"]  # Extract the beta coefficient
# Step 2: Calculate Alpha
# The RFR is already in the dataframe and adjusted for daily use.
annualized_market_return <- mean(Results_calspread2$Returns_SPX, na.rm = TRUE) * 252  # Annualize SPX returns
# Annualize strategy returns for comparison
annualized_strategy_return <- mean(Results_calspread2$Returns, na.rm = TRUE) * 252
# Annualize the mean daily RFR
annualized_rfr <- mean(Results_calspread2$RFR, na.rm = TRUE) * 360
# Calculate alpha using the CAPM formula
alpha_CalSpread2 <- annualized_strategy_return - (annualized_rfr + beta_calspread2 * (annualized_market_return - annualized_rfr))

# Output the results
cat("Beta: ", beta_calspread2, "\n")
cat("Alpha: ", alpha_CalSpread2, "\n")

# Add to other stats
new_rows <- data.frame(CalSpread2 = c(alpha_CalSpread2, beta_calspread2))
rownames(new_rows) <- c("Alpha", "Beta")

# Combine with the original results
# Use rbind to append the data, this assumes descrip_stats_results_calspread2 is a data frame with CalSpread2_75 as its column
descrip_stats_results_calspread2 <- rbind(descrip_stats_results_calspread2, new_rows)

# Round the CalSpread2 column
descrip_stats_results_calspread2$CalSpread2 <- round(descrip_stats_results_calspread2$CalSpread2, 4)
...
# Combine with the original results
# Use rbind to append the data, this assumes Results_calspread2 is a data frame with SPX as its column
...
# VPD Index
# Step 1: Run the regression to calculate Beta
model_vpdindex <- lm(Returns_VPD ~ Returns_SPX, data = Results_calspread2)
...
annualized_market_return <- mean(Results_calspread2$Returns_SPX, na.rm = TRUE) * 252  # Annualize SPX returns
# Annualize strategy returns for comparison
annualized_putindex_return <- mean(Results_calspread2$Returns_VPD, na.rm = TRUE) * 252
# Annualize the mean daily RFR
annualized_rfr <- mean(Results_calspread2$RFR, na.rm = TRUE) * 360
# Calculate alpha using the CAPM formula
...
# Combine with the original results
# Use rbind to append the data, this assumes Results_calspread2 is a data frame with VPD_Index as its column
...
combined_results <- cbind(descrip_stats_results_calspread2, results_vpd, results_spx)

# Rename the columns 
colnames(combined_results) <- c("CalSpread2", "VPD Index", "S&P 500")
