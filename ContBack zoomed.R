...
#Contango Backwardation Trigger zoomed
...
#Plot the results zoomed ####

# Filter data up to June 30, 2018
contback_data_filtered <- contback_data %>% filter(Date <= as.Date("2018-06-30"))

# Plot the results
# Calculate the upper limit for y-axis
upper_limit <- max(c(max(contback_data_filtered$MarginAccount, na.rm = TRUE), 
                     max(contback_data_filtered$margin_requirement_3, na.rm = TRUE),
                     max(contback_data_filtered$VPD, na.rm = TRUE),
                     max(contback_data_filtered$SPX, na.rm = TRUE)))
...
plot(contback_data_filtered$Date, contback_data_filtered$MarginAccount, type = 'l', 
...
lines(contback_data_filtered$Date, contback_data_filtered$margin_requirement_2, col = "deepskyblue4", lwd = 2)
lines(contback_data_filtered$Date, contback_data_filtered$VPD, col = "red", lwd = 2)
lines(contback_data_filtered$Date, contback_data_filtered$SPX, col = "black", lwd = 2)

# Custom x-axis labels for each year
year_ticks <- seq(from=min(contback_data_filtered$Date, na.rm=TRUE),
                  to=max(contback_data_filtered$Date, na.rm=TRUE), by="years")
...
contback_data_filtered <- contback_data_filtered %>%
  mutate(Returns = (MarginAccount - lag(MarginAccount)) / lag(MarginAccount))

# Replace the first NA value in Returns with 0 
contback_data_filtered$Returns[1] <- 0  



#Descriptive statistics ContBack zoomed ####
# Step 1: Calculate Simple Returns
contback_data_filtered <- contback_data_filtered %>%
  mutate(Returns = (MarginAccount - lag(MarginAccount)) / lag(MarginAccount))
...
summary_stats_contback <- contback_data_filtered %>%
  summarise(N.Obs = n(),
...
peak = contback_data_filtered$MarginAccount[1]
max_drawdown = 0

# Loop through the MarginAccount to find the maximum drawdown
for (value in contback_data_filtered$MarginAccount) {
...
skewness <- skewness(contback_data_filtered$Returns, na.rm = TRUE)
kurtosis <- kurtosis(contback_data_filtered$Returns, na.rm = TRUE)


# Step 5: Calculate Cumulative Return
cumulative_return <- last(contback_data_filtered$MarginAccount) / first(contback_data_filtered$MarginAccount) - 1


# Step 6: Annualize Returns and Volatility
annualized_return <- (1 + mean(contback_data_filtered$Returns, na.rm = TRUE)) ^ 252 - 1  # Assuming 252 trading days in a year
annualized_volatility <- sd(contback_data_filtered$Returns, na.rm = TRUE) * sqrt(252)


# Step 7: Calculate the Sharpe Ratio
# Calculate the Daily Excess Returns
contback_data_filtered$ExcessReturns <- contback_data_filtered$Returns - contback_data_filtered$RFR

# Calculate the Mean Daily Excess Retun and Standard Deviation for the Daily Returns 
mean_daily_excess_return_contback <- mean(contback_data_filtered$ExcessReturns, na.rm = TRUE)
sd_daily_excess_returns_contback <- sd(contback_data_filtered$Returns, na.rm = TRUE)
...
squared_negative_excess_returns <- ifelse(contback_data_filtered$ExcessReturns < 0, 
                                          contback_data_filtered$ExcessReturns^2, 
                                          0)
...
annualized_mean_daily_excess_return <- mean(contback_data_filtered$ExcessReturns, na.rm = TRUE) * 252
...
#Descriptive statistics VPD Index zoomed ####
...
contback_data_filtered$Returns_VPD[1] <- 0  


# Step 2: Generate Summary Statistics
summary_stats_vpd <- contback_data_filtered %>%
  summarise(N.Obs = n(),
...
# Step 3: Calculate Max Drawdown
# Initialize variables to track the peak and maximum drawdown
peak = contback_data_filtered$VPD[1]
max_drawdown = 0

# Loop through the VPD to find the maximum drawdown
for (value in contback_data_filtered$VPD) {
  # Update the peak if the current value is higher than the current peak
  if (value > peak) {
    peak = value
  }
...
# Step 4: Compute Skewness and Kurtosis
skewness <- skewness(contback_data_filtered$Returns_VPD, na.rm = TRUE)
kurtosis <- kurtosis(contback_data_filtered$Returns_VPD, na.rm = TRUE)


# Step 5: Calculate Cumulative Return
cumulative_return <- last(contback_data_filtered$VPD) / first(contback_data_filtered$VPD) - 1


# Step 6: Annualize Returns_VPD and Volatility
annualized_return <- (1 + mean(contback_data_filtered$Returns_VPD, na.rm = TRUE)) ^ 252 - 1  # Assuming 252 trading days in a year
annualized_volatility <- sd(contback_data_filtered$Returns_VPD, na.rm = TRUE) * sqrt(252)


# Step 7: Calculate the Sharpe Ratio
# Calculate the Daily Excess Returns_VPD
contback_data_filtered$ExcessReturns_VPD <- contback_data_filtered$Returns_VPD - contback_data_filtered$RFR

# Calculate the Mean Daily Excess Retun and Standard Deviation for the Daily Returns_VPD 
mean_daily_excess_return_vpd <- mean(contback_data_filtered$ExcessReturns_VPD, na.rm = TRUE)
sd_daily_excess_Returns_VPD <- sd(contback_data_filtered$Returns_VPD, na.rm = TRUE)
...
# Step 8: Calculate the Sortino Ratio
# Calculate the Downside Deviation
squared_negative_excess_Returns_VPD <- ifelse(contback_data_filtered$ExcessReturns_VPD < 0, 
                                              contback_data_filtered$ExcessReturns_VPD^2, 
                                              0)
...
annualized_mean_daily_excess_return <- mean(contback_data_filtered$ExcessReturns_VPD, na.rm = TRUE) * 252
...
#Descriptive statistics SPX zoomed ####
...
contback_data_filtered$Returns_SPX[1] <- 0  


# Step 2: Generate Summary Statistics
summary_stats_spx <- contback_data_filtered %>%
  summarise(N.Obs = n(),
...
# Step 3: Calculate Max Drawdown
# Initialize variables to track the peak and maximum drawdown
peak = contback_data_filtered$SPX[1]
max_drawdown = 0

# Loop through the SPX to find the maximum drawdown
for (value in contback_data_filtered$SPX) {
  # Update the peak if the current value is higher than the current peak
  if (value > peak) {
    peak = value
  }
...
# Step 4: Compute Skewness and Kurtosis
skewness <- skewness(contback_data_filtered$Returns_SPX, na.rm = TRUE)
kurtosis <- kurtosis(contback_data_filtered$Returns_SPX, na.rm = TRUE)


# Step 5: Calculate Cumulative Return
cumulative_return <- last(contback_data_filtered$SPX) / first(contback_data_filtered$SPX) - 1


# Step 6: Annualize Returns_SPX and Volatility
annualized_return <- (1 + mean(contback_data_filtered$Returns_SPX, na.rm = TRUE)) ^ 252 - 1  # Assuming 252 trading days in a year
annualized_volatility <- sd(contback_data_filtered$Returns_SPX, na.rm = TRUE) * sqrt(252)


# Step 7: Calculate the Sharpe Ratio
# Calculate the Daily Excess Returns_SPX
contback_data_filtered$ExcessReturns_SPX <- contback_data_filtered$Returns_SPX - contback_data_filtered$RFR

# Calculate the Mean Daily Excess Retun and Standard Deviation for the Daily Returns_SPX 
mean_daily_excess_return_spx <- mean(contback_data_filtered$ExcessReturns_SPX, na.rm = TRUE)
sd_daily_excess_Returns_SPX <- sd(contback_data_filtered$Returns_SPX, na.rm = TRUE)
...
# Step 8: Calculate the Sortino Ratio
# Calculate the Downside Deviation
squared_negative_excess_Returns_SPX <- ifelse(contback_data_filtered$ExcessReturns_SPX < 0, 
                                              contback_data_filtered$ExcessReturns_SPX^2, 
                                              0)
...
annualized_mean_daily_excess_return <- mean(contback_data_filtered$ExcessReturns_SPX, na.rm = TRUE) * 252
...
#Alpha and Beta calculation zoomed ####
# Strategy: 
# Step 1: Run the regression to calculate Beta
model_contback <- lm(Returns ~ Returns_SPX, data = contback_data_filtered)
...
annualized_market_return <- mean(contback_data_filtered$Returns_SPX, na.rm = TRUE) * 252  # Annualize SPX returns
# Annualize strategy returns for comparison
annualized_strategy_return <- mean(contback_data_filtered$Returns, na.rm = TRUE) * 252
# Annualize the mean daily RFR
annualized_rfr <- mean(contback_data_filtered$RFR, na.rm = TRUE) * 360
...
# VPD Index
# Step 1: Run the regression to calculate Beta
model_vpdindex <- lm(Returns_VPD ~ Returns_SPX, data = contback_data_filtered)
...
annualized_market_return <- mean(contback_data_filtered$Returns_SPX, na.rm = TRUE) * 252  # Annualize SPX returns
# Annualize strategy returns for comparison
annualized_putindex_return <- mean(contback_data_filtered$Returns_VPD, na.rm = TRUE) * 252
# Annualize the mean daily RFR
annualized_rfr <- mean(contback_data_filtered$RFR, na.rm = TRUE) * 360
# Calculate alpha using the CAPM formula
...
#All Descriptive Stats Table zoomed ####
